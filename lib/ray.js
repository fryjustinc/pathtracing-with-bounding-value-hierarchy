...
BIH.prototype = {
    ...
    generateSAHBuilder: function (tree) {
        return {
            ...
            getBestSplit: function (sortedArraysOfNodes, AABB1, totalWeight) {
                var parentSurfaceArea = AABB1.getSurfaceArea(),
                    cheapestAxis = -1,
                    cheapestIndex = -1,
                ...
                while (numberOfAxis-- > 0) {
                    leftAABB = new AABB();
                    rightAABB = new AABB();
                    leftAABB.min = AABB1.min;
                    leftAABB.max = AABB1.max;
                    rightAABB.min = AABB1.min;
                    rightAABB.max = AABB1.max;
                    ...
                    while (numberOfElements-- > 1) {
                        ...
                        currentLeftPlane = currentLeftPlane == Math.NaN ? currentEnd + .000000001 : Math.max(currentLeftPlane, currentEnd + .000000001);
                        currentRightPlane = currentStart - .000000001;
    ...
    buildFromArrayOfNodes: function (arrayOfNodes, deferredBuild) {
        ...
        this.i.mainmin = [-1.02,-1e-16,-1.04];
        this.i.mainmax = [1,1.59,.99000000000001];
        if (deferredBuild)
...
AABB.prototype = {
    ...
    expandByAABB: function (otherAABB) {
        var i;
        for (i = 0; i < this._numDimensions; i++) {
            this.min[i] = Math.min(this.min[i], otherAABB.min[i] - Number.EPSILON);
            this.max[i] = Math.max(this.max[i], otherAABB.max[i] + Number.EPSILON);
        }
    },
